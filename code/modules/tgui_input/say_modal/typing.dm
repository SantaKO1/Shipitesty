...
/mob/living/create_thinking_indicator()
	if(active_thinking_indicator || active_typing_indicator || stat != CONSCIOUS || !HAS_TRAIT(src, TRAIT_THINKING_IN_CHARACTER))
		return FALSE
	active_thinking_indicator = mutable_appearance('mod_celadon/_storge_icons/icons/qol/talk.dmi', "[bubble_icon]3", TYPING_LAYER)
...
/mob/living/create_typing_indicator()
	if(active_typing_indicator || active_thinking_indicator || stat != CONSCIOUS || !HAS_TRAIT(src, TRAIT_THINKING_IN_CHARACTER))
		return FALSE
	active_typing_indicator = mutable_appearance('mod_celadon/_storge_icons/icons/qol/talk.dmi', "[bubble_icon]0", TYPING_LAYER)
